...
/* LAB Performance v2 ----------------------------------------------------- */
.performance-kicker { color: var(--red); font-size: 12px; letter-spacing: .18em; margin-bottom: 5px; }
.dashboard-section .section-title { margin-bottom: 18px; }
.energy-bank-card { border-color: rgba(255,255,255,.16) !important; background: linear-gradient(180deg, rgba(255,255,255,.035), rgba(255,255,255,.012)), var(--panel) !important; }
.energy-bank-card:before { content:""; position:absolute; inset:0; pointer-events:none; background:radial-gradient(circle at 50% 0%, rgba(255,59,79,.12), transparent 48%); }
.gauge-wrap { width: 310px; }
.gauge-svg { filter: drop-shadow(0 12px 24px rgba(0,0,0,.5)); }
...
.gauge-needle { transform-origin:100px 95px; filter:drop-shadow(0 0 4px rgba(255,255,255,.65)); }
.gauge-score-wrap { bottom: 1%; }
.energy-bank-score { font-size: 48px; }
.dashboard-grid { grid-template-columns: repeat(4, minmax(0,1fr)); }
.gauge-card { border-color:rgba(255,255,255,.12); background:linear-gradient(180deg, rgba(255,255,255,.035), rgba(255,255,255,.012)), var(--panel); box-shadow:0 12px 30px rgba(0,0,0,.22); }
...
.polar-donut { width:130px; height:130px; border-radius:50%; margin:auto; display:grid; place-items:center; background:conic-gradient(var(--green) 0 calc(var(--low) * 1%), var(--orange) 0 calc((var(--low) + var(--mod)) * 1%), var(--red) 0 100%); box-shadow:0 0 28px rgba(37,212,122,.10); }
.polar-donut:after { content:""; width:92px; height:92px; border-radius:50%; background:var(--panel); position:absolute; }
.polar-donut { position:relative; }
.polar-center { position:relative; z-index:1; text-align:center; font-family:'Bayon',sans-serif; font-size:22px; }
.polar-center small { display:block; font-family:'Inter',sans-serif; font-size:9px; color:var(--grey-zone); letter-spacing:.08em; }
...
@media (max-width: 650px) { .dashboard-grid { grid-template-columns:repeat(2,1fr); } .readiness-strip { grid-template-columns:1fr; } .polarization-card { grid-template-columns:1fr; } }
...
<body>
 ...
 <line class="gauge-needle" x1="100" y1="95" x2="100" y2="25" stroke="var(--white)" stroke-width="2.5" stroke-linecap="round" transform="rotate({{ data.needle_angle }} 100 95)"/>
 <circle cx="100" cy="95" r="5.5" fill="var(--white)"/>
 <circle cx="100" cy="95" r="2" fill="var(--black)"/>
 </svg>
 ...
 <div class="polar-center">{{ data.zone_low_pct }}/{{ data.zone_high_pct }}<small>LOW / HIGH</small></div>
 </div>
